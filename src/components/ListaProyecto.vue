<template>
  <div class="lista">
    ...
    <table v-if="!isMobile" class="min-w-full bg-white border border-gray-300 rounded-lg shadow">
      ...
      <tbody>
        <tr v-for="item in itens" :key="item.id" @click="$emit('item-clicked', item.id)" class="hover:bg-gray-100 border-b border-gray-200 cursor-pointer">
          <td v-for="coluna in colunas" :key="coluna.key" class="py-2 px-4 text-gray-700">
            ...
            <div v-else-if="coluna.key === 'actions'" class="relative">
              ...
              <div v-if="menuAberto === item.id" class="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-md shadow-lg z-10">
                <button class="w-full px-4 py-2 text-left text-gray-700 hover:bg-gray-100 cursor-pointer" @click.stop="$emit('edit-item', item.id)">
                  Editar
                </button>
                <button class="w-full px-4 py-2 text-left text-red-500 hover:bg-gray-100 cursor-pointer" @click.stop="$emit('delete-item', item.id)">
                  Excluir
                </button>
              </div>
            </div>
    ...
    <div v-else class="space-y-2">
      <div v-for="item in itens" :key="item.id" @click="$emit('item-clicked', item.id)" class="bg-white border border-gray-300 rounded-lg p-3 flex justify-between items-center hover:bg-gray-100 cursor-pointer">
        <div class="flex-1">
    ...
    <div class="mt-4 flex flex-col md:flex-row justify-between items-center">
      <p class="text-gray-600">
        Mostrando {{ itens.length }} de {{ totalItems }} projetos
      </p>
